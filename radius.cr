...
def print_help()
  puts "\nradius.cr [-h] [-c0 nn] [-c1 nn] [-c2 nn] filename.\n "
  puts "  -h  : help"
...
# load file
infile = nil  # Use nil instead of false
skip_parm = false
...
source_contents = File.read(infile)
...
source_contents.each_line do |line|
  ...
  begin
    ...
    line.each_char.with_index(0) do |c, i|
      ...
      if i == 0 && c.to_s.strip.empty?
        buf = buf + " " * mnemonic_col_x
        x+=mnemonic_col_x
        label_done = true
        next                # SHORT CIRCUIT
      end
...
if std_out
  ...
  File.open(infile, "w") { |file| file.write(output_buf.to_slice) }
end
